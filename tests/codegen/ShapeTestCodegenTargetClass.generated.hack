...
final class ShapeTestCodegenTargetClass {
  ...
  public static function shapeOptionalAVecOfInt(mixed $htl_untyped_variable): shape(?'a' => vec<int>) { $out__1 = $htl_untyped_variable as shape(?'a' => mixed); if (Shapes::keyExists($out__1, 'a')) { $out__2 = vec[]; foreach (($out__1['a'] as vec<_>) as $v__2) { $out__2[] = $v__2 as int; } $out__1['a'] = $out__2; } else { Shapes::removeKey(inout $out__1, 'a'); } return $out__1; }
  public static function shapeOptionalAVecOfIntBStringWithExtraFields(mixed $htl_untyped_variable): shape(?'a' => vec<int>, 'b' => string, ...) { $out__1 = $htl_untyped_variable as shape(?'a' => mixed, 'b' => string, ...); if (Shapes::keyExists($out__1, 'a')) { $out__2 = vec[]; foreach (($out__1['a'] as vec<_>) as $v__2) { $out__2[] = $v__2 as int; } $out__1['a'] = $out__2; } else { Shapes::removeKey(inout $out__1, 'a'); } return $out__1; }
  public static function shapeWithUnicodeKey(mixed $htl_untyped_variable): shape('☃' => vec<string>) { $out__1 = $htl_untyped_variable as shape('☃' => mixed); $out__2 = vec[]; foreach (($out__1['☃'] as vec<_>) as $v__2) { $out__2[] = $v__2 as string; } $out__1['☃'] = $out__2; return $out__1; }
  public static function shapeWithQuoteInKey(mixed $htl_untyped_variable): shape('\'' => vec<string>) { $out__1 = $htl_untyped_variable as shape('\'' => mixed); $out__2 = vec[]; foreach (($out__1['\''] as vec<_>) as $v__2) { $out__2[] = $v__2 as string; } $out__1['\''] = $out__2; return $out__1; }
